...
@pytest.fixture
def encoder():
    return TrackSegEncoder(in_channels=N_PATCH_CHANNELS)


def test_forward_output_shape(encoder):
    """Forward should return (B, 1, H, W) logits."""
    x = torch.randn(2, N_PATCH_CHANNELS, 64, 64)
    out = encoder(x)
...
def test_segment_same_as_forward(encoder):
    ...
    x = torch.randn(1, N_PATCH_CHANNELS, 32, 32)
    encoder.eval()
    with torch.no_grad():
...
def test_variable_spatial_sizes(encoder):
    ...
    for size in [16, 32, 48, 64, 128]:
        x = torch.randn(1, N_PATCH_CHANNELS, size, size)
        out = encoder(x)

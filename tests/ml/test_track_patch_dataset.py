...
def test_patch_dataset_augment_flip():
    """Horizontal flip should negate easting channel (5)."""
    np.random.seed(42)
    patches = np.ones((1, 7, 4, 4), dtype=np.float32)
    # Set easting channel to a known pattern: [1, 2, 3, 4] across columns
    patches[0, 5] = np.array([[1, 2, 3, 4]] * 4, dtype=np.float32)
...
def test_patch_dataset_augment_noise():
    ...
    np.random.seed(0)
    patches = np.zeros((1, 7, 8, 8), dtype=np.float32)
    labels = np.array([0], dtype=np.float32)
    ds = TrackPatchDataset(patches, labels, augment=True, noise_std=0.1)

    # Run several times; channels 4-6 should stay at 0 (when not flipped)
    for _ in range(20):
        p, _ = ds[0]
        # Channels 4 (northing) and 6 (track_mask) should be unchanged
        # (channel 5 might be negated by flip but magnitude stays 0)
        assert torch.allclose(p[6], torch.zeros(8, 8))
...
def test_seg_dataset_augment_flips_target(seg_data):
    ...
    np.random.seed(42)
    patches = np.zeros((1, 7, 4, 4), dtype=np.float32)

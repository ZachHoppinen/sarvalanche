#!/bin/zsh
# Extract 2-channel SAR + 11-static (incl d_cr) patches for all labeled dates.
# Channels: SAR=[log1p_change, ANF], Static includes d_cr (cross-ratio change).
# Uses precomputed scene arrays for ~10x faster extraction.
set -e

SCRIPT="scripts/debris_pixel_classifier/v2/extract_patches_from_polygons.py"
SHAPES="local/issw/debris_shapes"
OUT="local/issw/v2_patches"
...
typeset -A BAN_DATES
BAN_DATES=(
  2022-12-01 "season_2022-2023_Banner_Summit.nc"
  2023-03-10 "season_2022-2023_Banner_Summit.nc"
  2024-01-10 "season_2023-2024_Banner_Summit.nc"
  2024-01-12 "season_2023-2024_Banner_Summit.nc"
  2024-02-04 "season_2023-2024_Banner_Summit.nc"
  2024-02-26 "season_2023-2024_Banner_Summit.nc"
  2024-02-29 "season_2023-2024_Banner_Summit.nc"
)

for DATE NC in "${(@kv)BAN_DATES}"; do
  if [[ -f "${OUT}/banner_2022_2023/${DATE}/labels.json" ]] || \
     [[ -f "${OUT}/banner_2023_2024/${DATE}/labels.json" ]]; then
    echo "=== Skipping Banner ${DATE} (already extracted) ==="
    continue
  fi
  # Figure out output subdirectory from NC filename
  if [[ "${NC}" == *"2022-2023"* ]]; then
    SUBDIR="banner_2022_2023"
  else
    SUBDIR="banner_2023_2024"
  fi
  echo ""
...
GAL_DATES=(
  ...
  2023-03-10 "season_2022-2023_Galena_Summit_&_Eastern_Mtns.nc"
  2024-01-10 "season_2023-2024_Galena_Summit_&_Eastern_Mtns.nc"
...
for DATE NC in "${(@kv)GAL_DATES}"; do
  if [[ "${NC}" == *"2022-2023"* ]]; then
    SUBDIR="galena_2022_2023"
  else
    SUBDIR="galena_2023_2024"
  fi
...
echo "=== All done ==="
echo "Patches in: ${OUT}"
echo "Next: assign confidence, then train"

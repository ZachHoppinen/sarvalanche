...
def run_all():
    ...
    for tau, cnn_path in sorted(TAU_CONFIGS.items()):
        ...
        row = {
            ...
            "mean_bump_symmetry": float(np.nanmean(result["bump_symmetry"].values[cm])),
...
def make_plots(df):
    ...
    # 2. Bump width
    ax = axes[0, 1]
    ax.plot(tau, df["mean_bump_width"], "o-", color="C0", linewidth=2)
    ax.set_ylabel("Mean bump width (steps)")
    ax.set_xlabel("Tau (days)")
    ax.set_title("Bump Width")
    ax.grid(True, alpha=0.3)

    # 3. Bump symmetry
    ax = axes[0, 2]
    ax.plot(tau, df["mean_bump_symmetry"], "o-", color="C2", linewidth=2)
    ax.set_ylabel("Mean bump symmetry")
    ax.set_xlabel("Tau (days)")
    ax.set_title("Bump Symmetry")
    ax.grid(True, alpha=0.3)

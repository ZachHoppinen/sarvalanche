...
Physical model:
  ...
  decay away from that reference date. For a real debris event, the
  probability time series forms a BUMP — rises as we approach the event
  (more pairs straddle it with high weight), peaks near the actual event
  date, then falls as we move past (those pairs get downweighted).

  Real debris:  smooth, symmetric bump that grows in both intensity and
                spatial extent as we approach the event, then shrinks.
  Noise:        sharp, asymmetric spike (one weird pair), no spatial growth,
                no smooth rise/fall.
...
Outputs:
  NetCDF with (y, x) maps:
    ...
    bump_width          - number of steps the bump spans above threshold
    bump_symmetry       - how symmetric the rise/fall is (1.0 = perfect)
    ...
    spatial_coherence   - fraction of neighbors with similar peak timing
    spatial_growth_sym  - symmetry of local spatial extent growth/shrink
...
def measure_bump_1d(
    ...
    """Measure bump shape around a peak.
    ...
    Returns
    -------
    width : int, number of consecutive steps above threshold around peak
    symmetry : float in [0, 1], ratio of shorter side to longer side
        (1.0 = perfectly symmetric rise and fall)
    ...
    # Find bump extent: contiguous region above threshold containing the peak
    ...
    # Symmetry: compare rise side (left to peak) vs fall side (peak to right)
    rise_len = peak_idx - left
    fall_len = right - peak_idx
    if rise_len + fall_len == 0:
        symmetry = 0.0
    else:
        shorter = min(rise_len, fall_len)
        longer = max(rise_len, fall_len)
        symmetry = shorter / longer if longer > 0 else 1.0

    # Smoothness: ratio of second-derivative energy to first-derivative energy
    # within the bump region
    bump_slice = series[left:right + 1].copy()
    nans = np.isnan(bump_slice)
    if nans.all() or len(bump_slice) < 3:
        return width, symmetry, 1.0

    # Interpolate NaNs for diff calculation
    ...
    return width, symmetry, smoothness
...
def find_peaks_batch(
    ...
    """Find peaks and measure bumps for all pixels.
    ...
    bump_symmetry = np.zeros(N, dtype=np.float32)
    bump_smoothness = np.zeros(N, dtype=np.float32)

    for i in range(N):
        peak_idx[i], peak_val[i] = find_peak_1d(cube[:, i], threshold)
        bump_width[i], bump_symmetry[i], bump_smoothness[i] = measure_bump_1d(
            cube[:, i], peak_idx[i], threshold
        )

    return peak_idx, peak_val, bump_width, bump_symmetry, bump_smoothness
...
def detect_spikes(
    ...
    bump_symmetry: np.ndarray,
    min_width: int = 2,
) -> np.ndarray:
    """Flag detections that look like transient noise rather than real bumps.

    A spike is a detection that:
    - Is only above threshold for 1 step (no rise/fall shape at all), OR
    - Has width < min_width (too narrow for the overlapping pair structure)

    Real debris with tau=6 should produce bumps spanning at least 2-3
    acquisition dates due to temporal weight overlap.

    Parameters
    ----------
    bump_width : (N,) int, width of bump above threshold
    bump_symmetry : (N,) float, symmetry score
...
def compute_confidence(
    ...
    """Combine bump shape and spatial metrics into a 0-1 confidence score.

    Factors:
    - Peak probability (higher = more confident)
    - Bump width (wider = more temporally supported)
    - Bump symmetry (symmetric rise/fall = real debris)
    - Bump smoothness (smooth = real, jagged = noise)
    - VV backscatter step magnitude
    - Spatial peak alignment (neighborhood peaks at same time)
    - Spatial bump amplitude (neighborhood probability is high, not just this pixel)
    """
    ...
    amp_score = np.clip(spatial_bump_amplitude / 0.5, 0, 1)  # 0.5 amplitude = full score
...
def run_temporal_onset(
    ...
    """Run temporal onset detection via bump analysis.

    For each candidate pixel (ever above threshold), finds the probability
    peak and characterizes the bump shape. Real debris produces a smooth,
    symmetric bump with rising then falling spatial extent. Noise produces
    narrow, asymmetric spikes.
    ...
    # ── Peak finding and bump characterization ────────────────────────
    log.info("Finding probability peaks and measuring bump shapes...")
    peak_idx, peak_val, bump_width, bump_sym, bump_smooth = find_peaks_batch(cnn_ts, threshold)
    ...
    # ── Spike detection ───────────────────────────────────────────────
    spike_flag = detect_spikes(bump_width, bump_sym, min_width=min_bump_width)
    ...
    confidence = compute_confidence(
        peak_val, bump_width, bump_sym, bump_smooth,
        vv_step,
    ...
    sym_map = np.full((H, W), np.nan, dtype=np.float32)
    ...
    log.info("  Mean bump width: %.1f steps", np.mean(bump_width))
    log.info("  Mean bump symmetry: %.3f", np.nanmean(bump_sym))
    ...
    result = xr.Dataset(
        {
            ...
            "bump_symmetry": (["y", "x"], np.full((H, W), np.nan, dtype=np.float32)),

"""Train Siamese debris detector on multi-zone data.

Same infrastructure as train_pairwise.py but uses SiameseDebrisDetector
which receives raw pre/post SAR images instead of pre-computed diffs.

Usage:
    conda run --no-capture-output -n sarvalanche python scripts/debris_pixel_classifier/train_siamese.py \
        --config scripts/debris_pixel_classifier/train_config_combined.yaml \
        --epochs 150 --auto-label-frac 0.25 \
        --out src/sarvalanche/ml/weights/siamese_debris_detector/best.pt
"""
...
from sarvalanche.ml.siamese_debris_classifier.dataset import (
    SiameseDebrisDataset, build_lazy_dataset,
)
from sarvalanche.ml.siamese_debris_classifier.model import SiameseDebrisDetector
...
def load_zone_datasets(zone_cfg, max_span_days=60, stride=64, month_range=(10, 4),
    ...
    val_ds = SiameseDebrisDataset.__new__(SiameseDebrisDataset)
    val_ds.__dict__.update(train_ds.__dict__)
    val_ds.__class__ = SiameseDebrisDataset
...
def main():
    ...
    parser.add_argument("--out", type=Path,
                        default=Path("src/sarvalanche/ml/weights/siamese_debris_detector/best.pt"))
    ...
    ckpt_meta = {
        "model_config": {
            "architecture": "SiameseDebrisDetector",
            "branch_ch": N_BRANCH,
            "static_ch": static_ch,

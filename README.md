...
The reframed architecture abandons the danger-period comparison framing entirely and instead lets the SAR data's native temporal resolution define the analysis units. Two parallel tau regimes run continuously through the season, producing two complementary output streams.
...
```
Every 6-day acquisition step:
    ├── tau = 24 days  →  CNN debris prediction
    │                     XGBoost path classification
    │                     → contributes to season inventory
    │                     → rolling 24-day detection count
    │
    └── tau = 3 days   →  CNN debris prediction
                          XGBoost path classification
                          WITH p_prior injected from 24-day result (same day)
                          → rolling 3-day detection count (timing signal)
```

Both streams run continuously and in parallel. The 24-day result on each step feeds directly as a soft prior into the 3-day classifier on the same step — not as a hard gate but as a Bayesian weight. No sequential season-level dependency is required.

---

### Terrain Polygon Foundation

All detection operates within pre-defined terrain polygons generated by the debris modeling system. This means the pipeline is not doing open-world change detection — it is answering a per-polygon question: *did this path run, and when?*

This has several implications:

- **Pass 1 recall is robust.** Integrating SAR signal within a known polygon across a full season with slow tau means even a single strong event should accumulate sufficient signal.
- **Pass 2 false positives are spatially bounded.** Noise in the fast pass can only manifest within polygons, substantially reducing the search space.
- **Outputs are directly comparable year-over-year** because the spatial domain is fixed.

The debris model also provides path aspect, elevation, and modeled maximum runout extent — enabling comparison of detected debris area against modeled maximum extent as a runout ratio metric.

---

### The p_prior Injection

The 24-day result on each step enters the 3-day XGBoost classifier as `p_prior_24d`. This can be implemented without retraining via a posterior adjustment:

```
p_final = p_3day * f(p_prior_24day)
```

Where `f` is a soft gating function (e.g. sigmoid or linear clamp). This approach:

- Requires no retraining of existing XGBoost weights
- Downweights but does not silently exclude paths with moderate 24-day confidence
- Keeps the two signals interpretably separate before combination

Paths with strong 24-day confidence carry full weight in the fast pass. Paths with weak 24-day confidence are downweighted but not excluded — preserving recall for marginal cases.

---

### Output Streams

#### Output A — Annual Spatial Inventory

Produced from the 24-day tau stream, accumulated across the full season.

| Field | Type | Description |
|---|---|---|
| `ran_this_season` | bool | Did this path show confirmed debris signal? |
| `slow_confidence` | float [0–1] | Mean 24-day XGBoost confidence across active windows |
| `cnn_debris_outline` | geometry | Mean CNN debris outline across active windows |
| `debris_area_fraction` | float | Detected area / modeled maximum extent |
| `n_active_windows` | int | Number of 6-day steps with confirmed signal |

One record per terrain polygon per season. Directly comparable year-over-year.

#### Output B — Rolling Detection Curves

Two time series, both at 6-day step resolution:

**24-day rolling curve** — number of paths (or total debris area) active in each step's 24-day tau window. Smooth and low-noise. Captures seasonal accumulation and integrated activity level.

**3-day rolling curve** — number of paths showing fresh signal in each step's 3-day tau window, weighted by the 24-day prior. Noisier but temporally sharp. Identifies activity pulses within the season.

The 3-day curve sitting on top of the 24-day curve provides a two-layer seasonal narrative: the slow curve shows accumulated activity context, the fast curve shows when pulses actually occurred (to within ±12 days, the hard limit of the acquisition interval).

---

### Temporal Precision Caveat

The 3-day tau does not provide 3-day timing precision. With 12-day acquisitions, `tau=3` means the nearest acquisition receives dominant weight (~exp(0) = 1.0) while a 12-day-old pair receives near-zero weight (~exp(-4) ≈ 0.018). In practice the fast pass performs nearest-acquisition selection rather than true weighted fusion. Timing precision is bounded by the acquisition interval: **±12 days**.

This should be stated explicitly in any reporting of the rolling timing curve.

---

### Season Example: 2024–2025

For a full season run:

1. Ingest all Sentinel-1 acquisitions across the season
2. At each 6-day step, compute both tau=24 and tau=3 weighted feature stacks within all terrain polygons
3. Run CNN debris predictor on both stacks
4. Run XGBoost on 24-day features → `p_24day` per polygon
5. Run XGBoost on 3-day features with `p_prior = p_24day` → `p_3day_weighted` per polygon
6. Accumulate 24-day results into season inventory (Output A)
7. Record both rolling counts at each step (Output B)

End of season outputs: one spatial inventory layer + two time series curves per polygon (or aggregated across study area).

---

### Scientific Questions This Framing Supports

- Which terrain polygons run consistently across seasons? (inventory spatial persistence)
- Do certain aspects or elevation bands dominate activity in a given season? (inventory stratification)
- Can distinct activity pulses be identified within a season from the 3-day curve? (intra-season temporal structure)
- Does the shape of the rolling activity signal correlate with meteorological forcing windows? (loading-response analysis)
- Do paths run closer to full modeled runout extent in high-activity seasons? (runout ratio vs. season intensity)
